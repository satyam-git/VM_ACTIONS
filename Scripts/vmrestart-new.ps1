    ...
    try {
        ...
        if ($null -eq $vm) { 
            ...
            switch ($action) {
                ...
                "restart" { 
                    if ($vm.powerState -eq "off") { $Status = "it is off, please power on" } 
                    else {
                        Write-Host "Initiating restart (Strike 1) for $vmName..."
                        Set-NTNXVMPowerState -Vmid $vm.uuid -Transition ACPI_REBOOT
                        Start-Sleep -Seconds 40
                        
                        # Validate State & Strike 2
                        $currentVm = Get-NTNXVM -Vmid $vm.uuid
                        if ($currentVm.powerState -ne "on") {
                            Write-Host "Strike 2: Restart failed, retrying..."
                            Set-NTNXVMPowerState -Vmid $vm.uuid -Transition ACPI_REBOOT
                        }
                        $Status = "successful"
                    }

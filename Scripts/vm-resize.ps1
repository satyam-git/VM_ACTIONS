...
# --- 2. CONSTRUCT PAYLOAD ---
$resources = $currentVm.spec.resources
$resources.num_vcpus = [int]$data.CPU_size
$resources.memory_size_mib = [int]$data.mem_size * 1024

if ($data.disk_action -eq "add") {
    $maxIndex = 0
    foreach ($d in $resources.disk_list) {
        if ($d.device_properties.disk_address.device_index -gt $maxIndex) { $maxIndex = $d.device_properties.disk_address.device_index }
    }
    $resources.disk_list += @{ 
        device_properties = @{ device_type = "DISK"; device_address = @{ adapter_type = "SCSI"; device_index = ($maxIndex + 1) } }; 
...
# --- 3. APPLY ---
$body = @{
    spec = @{ name = $currentVm.spec.name; resources = $resources }
